          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <div>
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                Repository-specific suggestions generated by GitPilot analysis:
              </p>
            </div>
            {Array.isArray(intelligence?.suggestions) && intelligence.suggestions.length > 0 && (
              <span className="badge badge-primary">{intelligence.suggestions.length} ACTION ITEMS</span>
            )}
          </div>

          {aiLoading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.25rem' }}>
              <Skeleton height="180px" borderRadius="1.25rem" />
              <Skeleton height="180px" borderRadius="1.25rem" />
              <Skeleton height="180px" borderRadius="1.25rem" />
            </div>
          ) : (Array.isArray(intelligence?.suggestions) && intelligence.suggestions.length > 0) ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.25rem' }}>
              {intelligence.suggestions.map((s, idx) => (
                <RecommendationCard
                  key={idx}
                  title={s.title}
                  category={s.category}
            ...
            <div style={{ padding: '2.5rem 1.5rem', textAlign: 'center', border: '1px dashed var(--border-color)', borderRadius: '1.25rem', background: 'var(--bg-secondary)' }}>
              <Sparkles size={28} style={{ color: 'var(--text-muted)', marginBottom: '0.75rem' }} />
              <p style={{ fontSize: '0.95rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.35rem' }}>
                No recommendations yet for this repository
              </p>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', margin: 0 }}>
                Run an AI analysis to generate repository-specific recommendations.
              </p>
            </div>
          )}
        </motion.div>
